...
The `Prettier` Addon in **Servest** automatically sets up code formatting in your Servest project using Prettier — ensuring a consistent and clean code style across your team.
...
- `.prettierrc` → Contains your Prettier configuration options (like tab width, quotes, and semicolons).
- `.prettierignore` → Lists files and folders to skip during formatting (like `node_modules` or build directories).
...
3. **Config Creation:**  
   Runs `addPrettierConfig()` to create `.prettierrc` and `.prettierignore` files if they don’t exist.
...
- You can safely re-run `npx servest@latest add prettier` — it will skip existing configurations without overwriting your setup.
- Works seamlessly with the [ESLint Addon](./eslint) for combined linting and formatting workflows.
- Only supports Node.js-based frameworks for now — cross-language support (e.g., Python, Go) is planned for future versions.
...
Once complete, your project will have automatic formatting capabilities — keeping your codebase beautiful and consistent.
